...
Реализован шаблон `WeakPtr` — аналог `std::weak_ptr`, а также функция `MakeShared` — аналог `std::make_shared`.

Для поддержки "слабых" ссылок предлагается вынести счетчик обычных (сильных) ссылок `strong_count` в отдельную структуру `Counter`, в которую дополнительно добавлено поле `weak_count` для подсчета количества слабых указателей, указывающих на данный объект. Как и ранее, сам счетчик (объект `Counter`) находится в динамической памяти, а указатель на него храниться в соответствующих объектах `SharedPtr` и `WeakPtr`. Логика освобождения ресурса следующая:
...
Дополнительно реализована внешняя шаблонная функция `MakeShared`, принимающую произвольное число параметров и возвращающая `SharedPtr` на объект созданный с помощью данных параметров конструктора. (на практике `std::make_shared` позволяет выделить счетчики и объект единым блоком в памяти, что часто повышает эффективность программы).
